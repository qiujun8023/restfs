...
// 关闭 WithUnsafe（默认）：goldmark 会转义 Markdown 中的 raw HTML，防止 README 注入脚本
var md = goldmark.New(
	goldmark.WithExtensions(extension.GFM),
)

var (
	dirCSSMinified string
...
func init() {
	...
	if err := m.Minify("text/css", &buf, strings.NewReader(rawDirCSS)); err == nil {
		dirCSSMinified = buf.String()
	} else {
		dirCSSMinified = rawDirCSS
	}

	dirTmpl = template.Must(template.New("dir").Funcs(template.FuncMap{
		"formatSize":      formatSize,
		"safeHTML":        func(s string) template.HTML { return template.HTML(s) },
		"splitBreadcrumb": splitBreadcrumb,
		"css":             func() template.CSS { return template.CSS(dirCSSMinified) },
	}).Parse(dirTmplStr))
}

// renderMarkdown 将 Markdown 转换为 HTML（raw HTML 会被转义，见上方 goldmark 配置）
...
func splitBreadcrumb(displayPath string) []breadcrumbPart {
	if displayPath == "/" {
	...
	parts := make([]breadcrumbPart, 0, len(segments)+1)
	parts = append(parts, breadcrumbPart{Name: "~", Href: "/"})
...
{{range splitBreadcrumb .Path}}{{if .IsLast}}<span class="current">{{.Name}}</span>{{else}}<a href="{{.Href}}">{{.Name}}</a><span class="sep">/</span>{{end}}{{end}}
</nav>
<div class="card">
<div class="table-wrapper">
<table>
<thead><tr><th>Name</th><th class="right">Size</th><th class="right hide-mobile">Modified (UTC)</th></tr></thead>
<tbody>
{{if ne .Path "/"}}<tr><td><div class="name-cell"><span class="icon">↩</span><a class="name-link dir-link" href="../">..</a></div></td><td></td><td class="hide-mobile"></td></tr>{{end}}
{{range .Entries}}<tr>
<td><div class="name-cell">{{if eq .Type "directory"}}<span class="icon">📁</span><a class="name-link dir-link" href="{{.Path}}">{{.Name}}</a>{{else}}<span class="icon">📄</span><a class="name-link" href="{{.Path}}">{{.Name}}</a>{{end}}</div></td>
<td class="size right">{{if eq .Type "directory"}}—{{else}}{{formatSize .Size}}{{end}}</td>
<td class="mtime right hide-mobile">{{.Modified}}</td>
</tr>{{end}}
{{if eq (len .Entries) 0}}<tr><td colspan="3" class="empty">Empty directory</td></tr>{{end}}
</tbody>
</table>
</div>
</div>
{{if .ReadmeHTML}}<div class="card readme-card">
